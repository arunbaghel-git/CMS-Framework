...
>    shuffle, client). ⏭️ **Home speed jaari** (93 mobile local) — agla: markup chhota. `:3000` pe abhi **production** server
>    (`pnpm start`) chal raha hai, dev band hai.
...
## ⏭️ 18 Sep shaam — Layout ▸ Spacing ban gaya (D-100)

Client ne Layout ke naye options me se **sirf do** chune (_"ye almost 100% site par use honge"_). Baaki options (box
padding, sidebar width, article width, image kone, form fields, header shadow, button caps) **nahi**.

- **Settings ▸ Layout ▸ Spacing** (Page Width ke neeche): Section spacing desktop/mobile (home ke band, neela banner,
  package CTA) · Block spacing desktop/mobile (package/tour/Section layout ke card se card, **aur** Default page/blog post
  ke card ke andar — anupaat me, apna khaana nahi) · Cards gap **rows + columns alag** (client ne do maange)
- `globals.css`: `--space-section` · `--space-block` · `--space-block-in` · `--gap-card-row` · `--gap-card-col`. Script se
  milaan: sirf cards gap badla (10/11/12 → 14, client ne common maana), baaki bilkul wahi
- Tests: shared 16/16, API settings pass. Dev server pe naya CSS pahunchta dekha (curl) — **aankh se render nahi dekha**
- Usi shaam: Settings ▸ Layout/Fonts/Colours ke panel **band/khule** hote hain (`Panel.jsx`), Preview nahi. Text Sizes ki
  hint theek ki (weight/line/spacing ab har jagah lagte hain, D-99 §3). Vite se compile dekha, **browser me nahi**
- ⚠️ `theme-fonts.test.js` ka ek test fail — `.hf-stat span` pe client ka apna edit (size small, weight abhi
  `--xsmall-w`). Client ko bataya; bina kahe nahi chheda

**Aaj ka baaki:** D-99 §1–§3 commit ho chuke (`3cb28a7`, `6a207a7`, `186f6c0`), push nahi. Fonts 86% screen + 14% Custom
CSS = 100% admin; Colours 97% + 3% (WhatsApp, map, Not-included — client: fixed theek).
⚠️ `.claude/` git se bahar hai (client ke commit cb2a9aa/e62296b) — client ka jawab baaki; docs disk pe hain.
⚠️ C: pe ~0.8 GB bachi (18 Sep).

---

## ⏭️ 18 Sep — headings common ho gaye (D-99)

Client ne page-wise dekh kar tay kiya: **h1 40 · h2 25 · h3 16** (tablet/mobile ke saath), apwaad h3 19 (home form + map)
aur 24 (blog feature card), footer h2 11.5. `globals.css` + `theme-fonts.js` (H1–H6 = sirf tag ka token, baaki
`FONT_FIXED_TOKENS`) + **migration 026** (DB me chal chuki). Desktop render dekha (home, blog, tour) — theek.
Usi din **§3**: Fonts ke weight/line/spacing har level-size rule pe lagte hain (86% font CSS admin se). Usi din **§2**: text ke 3 naap (Body 14 · Small 13 · Extra small 11), bade non-heading h1/h3/19 pe — **41 → 17 token**.
⚠️ Mobile aankh se nahi dekha. ⚠️ `.claude/` git se bahar hai (client ke do commit cb2a9aa/e62296b) — client se poochha hai.
⏭️ Agla: client batayega (A-32 "Text Elements" ab chhota kaam — 17 token).

---

## (17 Sep raat) ⏭️ Nayi session yahan se shuru kare — **Fonts ka dhaancha: Text Elements pe faisla**
...
- **16 Sep — custom editor ke tabs** (§29): `sw-tab`/`sw-panel`/`i-<key>` — behaviour theme me, content
  editor me. Island map isi se chalta hai; naya block nahi bana (client)
