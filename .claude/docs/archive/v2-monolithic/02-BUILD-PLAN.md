...
- **Patterns** — 6-8 ready-made sections (hero, features, CTA, testimonial, pricing,
  contact) jo ek click me insert ho. *(Naam "section templates" se "Patterns" kiya —
  insert hote hi copy ban jaate hain)*
...
- Security audit: rate limiting, file type/size validation, secrets audit
  *(XSS sanitize, Mongo injection guard, upload hardening ab pehle ke phases me hain)*
...
3. `entries` aur `blocks` ka JSON contract likh ke freeze karo — poora system isi pe
   ...
   block ki *list* nahi — wo Phase 5 me asli design se nikalegi
