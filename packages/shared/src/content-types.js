...
const PAGE_FIELDS = [
  {
     ...
     *
     * `default` = `page-template-text.html` (poora content ek safed card me, D-95).
     * `sections` = `contact-us.html` (har block apna card, FAQ accordion, na stat rail na hero button).
     *
     * ⚠️ **Ye ek field hai, alag content type nahi.** Pehle type banaya gaya tha (`sectionPage`) aur client
     * ne wo mana kiya: unhone _"Add New Page ke sidebar me template dropdown"_ maanga tha. Field wala raasta
     * behtar bhi nikla — ek hi list, aur template **baad me badla ja sakta hai** (type badalna allowed hi
     * nahi hai).
     *
     * Khaali = `default`, isliye purane saare pages jaise the waise rehte hain aur koi migration nahi lagi.
     */
    key: 'template',
    type: 'select',
    label: 'Template',
    help: 'Default keeps everything in one card. Section layout gives each block its own card.',
  },
  {
    key: 'subheading',
    type: 'richText',
