...
import { HOTEL_CATEGORY_LABEL, formatPrice, isEmptyHtml } from '@cms/shared'
...
export function HotelsSection({ hotels, label }) {
   ...
   */
  if (tabs.length === 0) {
    if (isEmptyHtml(label?.description)) return null
...
export function AddOns({ addOns, label }) {
  ...
  if (!addOns?.length && isEmptyHtml(label?.description)) return null
